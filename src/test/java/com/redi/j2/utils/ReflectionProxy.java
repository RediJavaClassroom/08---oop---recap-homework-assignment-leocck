...
public abstract class ReflectionProxy {
    ...
    public boolean isPropertyFinal(String name) {
        Class<?> targetClass = getTargetClass();
        if (targetClass == null || name == null) {
            return false;
        }
    ...
    protected <T> T getPropertyValue(String propertyName) {
        ...
        try {
            ...
            // trying the parent class (in case it's a mockito instance)
            try {
                Field field = target.getClass().getSuperclass().getDeclaredField(propertyName);
                field.setAccessible(true);
                return (T) field.get(target);
            } catch (Exception ex) {
                return null;
